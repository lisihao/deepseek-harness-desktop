...
desktop package 拥有普通 Host 与 Web Client 两个 face。它的 Client face 会在两种模式下校验 Host 提供的模式与平台 marker，然后在完整应用 surface 下方挂载一条 Desktop 自有的单行产品页脚。页脚显示打包产品版本与 Solar 定位，不占用 sidebar slot。除此以外，兼容模式不会注册 service、slot 或呈现；高级模式则安装下文所述的 desktop layout service 与 root 呈现。两种模式下，第三方 Web client 都继续使用普通 DSH 模块图。
