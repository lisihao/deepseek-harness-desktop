...
The desktop package has normal Host and Web Client faces. Its Client face validates the Host-supplied mode and platform markers in both modes, then mounts one Desktop-owned, single-line product footer below the complete application surface. The footer shows the packaged product version and Solar positioning without consuming a sidebar slot. Compatibility otherwise returns without registering services, slots, or presentation; advanced mode installs the desktop layout service and root presentation described below. Third-party Web clients continue to use the ordinary DSH module graph in both modes.
